...
| 交互 | 重新生成 | **对话式改图**：增删卡片 / 连线 / 分组容器，原子操作、实时预览、逐条回执 |
| 生态 | 无 | 知乎 OAuth / 热榜 / 搜索 / 关注流 / 收藏夹深度打通 |

## 核心能力

- 🔭 **观点对照图** — 输入问题，自动检索知乎回答，提取各方论点、证据与立场，生成左右阵营对照 + 底部共识通栏的对照图。你关注的答主会被自动高亮。
- 🗺 **学习路线图** — 领域关键词 → 入门 / 进阶 / 避坑的泳道路径图，可以直接从你的知乎收藏夹生成。
- 📝 **文章摘要图** — 勾选知乎收藏夹里的回答 / 文章（或搜索后勾选），提炼中心主题与左右分支的思维导图。
...
**创新度（25%）** — 区别于红海的「文章转导图」，本项目做了三件不常见的事：① 多源观点对照与阵营化表达；② 图不是终点——Agent 以受约束的语义操作协议（而非自由文本）连续修改图表的**结构**（增删卡片、连线、分组容器），不止于改样式；③ 生成链路是可解释的 Harness 编排，每个阶段对用户可见（见下文架构）。

**完成度（25%）** — 生产环境已上线并完成真实 OAuth 闭环验证，190 项测试（184 通过 / 0 失败 / 6 OAuth 场景跳过）、TypeScript 零错误、ESLint 零错误、16 条路由全量构建通过。
...
- **Agent 2.0 改图协议** — 语义化 GraphChange 替代自由文本 patch：改名 / 强调 / 合并 / 删除之外，还支持**结构操作**——新增卡片（add_node）、大卡片容器（add_group 空标签 → 渲染层按成员卡包围盒画虚线大框）、增删连线（add_edge / remove_edges，删除记入 metadata.removedEdges 且对泳道站间箭头、胶囊→共识等装饰箭头同样生效，说「恢复连线」可逆）。风险分级 + 原子提交 + 高危操作预览确认；结构变更触发受控重排，纯文字修改与配色切换保留用户坐标（局部重渲染）。
...
| `OPENAI_COMPAT_BASEURL` / `OPENAI_COMPAT_API_KEY` / `OPENAI_COMPAT_MODEL` | OpenAI 兼容推理端点（Planner / Synthesizer / 看山助手使用） |
...
│   │   ├── agent/          # 看山助手对话改图（含高危操作预览确认 commit 路径）
...
└── lib/
    ...
    │   ├── layouts.ts      #   6 种确定性布局 + 动态卡高防重叠 + 分组容器/连线隐藏渲染
    │   └── patch.ts        #   Agent 语义操作协议
    ├── agent/              # Agent 2.0（意图路由 / 决策生成 / 风险分级 / 原子应用）
...
当前基线：190 tests（184 pass / 0 fail / 6 OAuth 场景在有凭证环境跳过）· tsc 0 error · lint 0 error · 16 路由构建全过。Harness 各层（planner / executor / sources / synthesizer / layouts / patch / compat）与 Agent 2.0（router / decide / apply）均有独立回归测试，布局模板含「零重叠 / 零超宽 / 文字不出卡 / 分组容器包住成员 / 被删连线不渲染」断言。
