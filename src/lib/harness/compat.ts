...
export function viewpointToKnowledgeGraph(value: LegacyViewpointInput | KnowledgeGraph): KnowledgeGraph {
  ...
  value.viewpoints.forEach((v, i) => nodes.push({ id: `viewpoint-${i + 1}`, label: v.stance, description: v.summary, citations: [...v.sources], group: "viewpoints", metadata: { evidence: [...v.evidence], authors: [...v.authors] } }));
...
export function roadmapToKnowledgeGraph(value: RoadmapGraph | KnowledgeGraph): KnowledgeGraph {
  ...
  const groups = value.stages.map((stage, si) => {
    const ids: string[] = [];
    stage.items.forEach((item, ii) => { const id = `stage-${si + 1}-item-${ii + 1}`; ids.push(id); nodes.push({ id, label: item.topic, description: item.detail, citations: item.source ? [item.source] : [], group: `stage-${si + 1}` }); });
...
export function knowledgeGraphToViewpoint(graph: KnowledgeGraph): ViewpointGraph {
  const viewpoints = graph.nodes.filter((n) => n.group === "viewpoints").map((n) => ({ stance: n.label, summary: n.description, evidence: Array.isArray(n.metadata?.evidence) ? n.metadata.evidence as string[] : [], authors: Array.isArray(n.metadata?.authors) ? n.metadata.authors as string[] : [], sources: n.citations }));
