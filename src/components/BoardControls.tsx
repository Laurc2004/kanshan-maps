...
function PaletteDropdown({ value, onChange }: { value: PaletteId; onChange: (palette: PaletteId) => void }) {
  ...
  return (
    <div ref={ref} className="relative" data-testid="palette-dropdown">
      ...
      {open && (
        <ul role="listbox" aria-label="配色" className="absolute right-0 top-9 z-50 min-w-32 overflow-hidden rounded-[10px] border border-[#ecece8] bg-white py-1 shadow-[0_4px_16px_rgb(0_0_0/0.08)]">
...
/**
 * 嵌入 Excalidraw renderTopRightUI 的画板控件：配色下拉 + 保存图片。
 * Island 同款包装，与画板原生 UI 协调。
 */
export function BoardControls({ graph, busy, makePng, onPaletteChange }: {
  ...
  return (
    <div className="ks-board-ui flex items-center gap-1 rounded-[10px] border border-[#ecece8] bg-white p-1 shadow-[0_2px_10px_rgb(0_0_0/0.05)]" data-testid="board-topright-ui">
